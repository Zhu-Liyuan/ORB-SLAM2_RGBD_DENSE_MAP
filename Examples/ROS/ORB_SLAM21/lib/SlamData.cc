...
SlamData::SlamData(ORB_SLAM2::System* pSLAM, ros::NodeHandle *nodeHandler, bool bPublishROSTopic)
{
    ...
    dense_map_cloud_pub = (*nodeHandler).advertise<sensor_msgs::PointCloud2>("point_cloud_dense",1);
...
void SlamData::PublishDenseMap()
{
    sensor_msgs::PointCloud2 DenseMapROS;

    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr temp_points;
    std::cout<< "1"<<std::endl;
    temp_points=mpSLAM->mpPointCloudMapping->getGlobalMap();
    std::cout<< "2"<<std::endl;
    
    // for (size_t i = 0; i < temp_points->points.size(); i++)
    // {
    //     Eigen::Vector4f p_temp, p_temp_t;
    //     p_temp(0) = temp_points->points[i].x;
    //     p_temp(1) = temp_points->points[i].y;
    //     p_temp(2) = temp_points->points[i].z;
    //     p_temp(3) = 1;
    //     p_temp_t = mTrans_cam2ground * p_temp;	
    //     temp_points->points[i].x = p_temp_t(0);
    //     temp_points->points[i].y = p_temp_t(1);
    //     temp_points->points[i].z = p_temp_t(2);
    // }
    
/* pcl::PCLPointCloud2 pcl_pc3;
        pcl::toPCLPointCloud2(*temp_points, pcl_pc3);
 // pcl::PointXYZRGBA -> pcl::PCLPointCloud2
        pcl_conversions::fromPCL(pcl_pc3, DenseMapROS);  
// pcl::PCLPointCloud2 -> sensor_msgs::PointCloud2
*/
    pcl::toROSMsg(*temp_points,DenseMapROS);
    std::cout<< "3"<<std::endl;
    DenseMapROS.header.frame_id = "world";////////////////////////////////////////////////////
    DenseMapROS.header.stamp = ros::Time::now();
    dense_map_cloud_pub.publish(DenseMapROS);
